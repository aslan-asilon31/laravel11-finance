  ...
  <x-card :title="$title" :url="$url" shadow separator class="border shadow">
    ...
    <table>
      ...
      <tbody>
            ...
            <td>
              <label class="inline-flex items-center">
                <input type="checkbox" class="form-checkbox h-5 w-5 text-blue-600"
                  wire:click="toggleSold({{ $product->id }})" {{ $product->is_sold ? 'checked' : '' }}>
        ...
        <tr>
          <td colspan="2">{{ $this->products->links() }}</td>
        </tr>
      </tbody>
    </table>
    ...
    <x-modal wire:model="modalProduct" title="Product Show" class="backdrop-blur">
      ...
      <div class="mb-3">
        <x-input label="Name" value="{{ $product->name }}" id="masterForm.name" name="masterForm.name"
          placeholder="Name" readonly />
      </div>

      <div class="mb-3">
        <x-input label="Detail" value="{{ $product->detail }}" id="masterForm.name" name="masterForm.name"
