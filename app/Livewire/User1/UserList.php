...
class UserList extends Component
{
    ...
    public function headers(): array
    {
        return [
            ...
            ['key' => 'email', 'label' => 'Email', 'sortBy' => 'email',  'class' => 'whitespace-nowrap  border-1 border-l-1 border-gray-300 dark:border-gray-600 text-left'],
    ...
    public function rows(): LengthAwarePaginator
    {
        ...
        $query->when($this->search, fn($q) => $q->where('name', 'like', "%{$this->search}%"))
            ...
            ->when(($this->filters['email'] ?? ''), fn($q) => $q->where('email', $this->filters['email']))
    ...
    public function exportFastExcel()
    {
        ...
        // Export all users
        (new FastExcel($user))->export('user.xlsx');
    }
    ...
    {
        return view('livewire.user1.user-list');
    }
}
